`timescale 1ns / 1ps
module cnn(clk,reset,w_conv,w_fc,pic,b_conv,b_fc,out,start,valid);
parameter WIDTH = 8;
parameter PIX_W = 6;
parameter PIX_H = 6;
parameter FILTER = 3;
...
input [WIDTH*((PIX_W-FILTER+1)>>1)*((PIX_H-FILTER+1)>>1)*CONV1*OUTNODE-1:0] w_fc;
...
dense_layer #(.WIDTH(WIDTH),.INNODE(((PIX_W-FILTER+1)>>1)*((PIX_H-FILTER+1)>>1)*CONV1),.OUTNODE(OUTNODE)) i_dense0(.clk(clk), .reset(reset), .weight(w_fc), .in(sum23), .bias(b_fc), .out(sum34), .start(valid23), .valid(valid34));
