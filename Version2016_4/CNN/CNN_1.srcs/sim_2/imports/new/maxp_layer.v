...
module maxp_layer(clk,reset,feature_map,out,valid);
parameter WIDTH = 8;
parameter PIX_W = 4;
parameter PIX_H = 4;
parameter FILTER = 2;
...
output [WIDTH*(PIX_W >> 1)*(PIX_H >> 1):0] out;
output valid;
...
always@(*) begin
    ...
    if(state_r == STAR) begin
        ...
        cntrow_w = 2;
        cntcol_w = cntcol_r + 2;
        for (i=0;i<PIX_W*PIX_H-FILTER+1;i=i+1) begin
            if(PIX_W%2 == 0) image_w[i+PIX_W+2] = image_r[i];
            else image_w[i+PIX_W+3] = image_r[i];
        end
    end else begin
        valid = 1;
    end
